...
for (let i in color16s) {
  ...
  termColor[
    'bright' + (color16s[i].charAt(0).toUpperCase() + color16s[i].slice(1))
  ] = (text) => {
    return `\x1b[9${i}m${text}\x1b[0m`
  }
  termColor[
    'bg' + (color16s[i].charAt(0).toUpperCase() + color16s[i].slice(1))
  ] = function (text) {
    return `\x1b[4${i}m${text}\x1b[0m`
  }
  termColor[
    'bgBright' + (color16s[i].charAt(0).toUpperCase() + color16s[i].slice(1))
  ] = (text) => {
