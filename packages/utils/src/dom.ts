    ...
    element.addEventListener(
      ...
      (e: Event) => {
        ...
        if (flag !== '__stop__' || eventExtra.eventStop) {
          fn(e as any, target, flag);
        }
      },
      eventExtra,
    );
  } else {
    element.addEventListener(listener, fn as any, eventExtra);
  }
}
 ...
 */
export function queryHideNodeSize(hideNode: string | HTMLElement, parent = document.body) {
  // 计算折叠菜单的高度
